...
socks.setdefaultproxy(socks.PROXY_TYPE_SOCKS4, "192.168.1.231", 8080)
...
# Example taken from http://docs.python.org/3/library/xmlrpc.client.html and
# adopted for pwclient
class ProxiedTransport(xmlrpclib.Transport):
    def set_user_params(self, username = None, password = None, use_https = False):
    ...
    def send_request(self, connection, handler, request_body):
        print "%s" % request_body
        print "https://%s%s" % (self.realhost, handler)
...
def main():
    ...
    use_proxy = False
    if config.has_option('proxy', 'use_proxy'):
	...
	p.set_user_params(
	     ...
	     config.get('auth', 'password'),
	     use_https)
	if config.has_option('proxy', 'use_proxy') and\
                config.has_option('proxy', 'proxy'):
		p.set_proxy(config.get('proxy', 'proxy'))
	        transport = p

    try:
        rpc = xmlrpclib.ServerProxy(url, transport = transport)
    except:
